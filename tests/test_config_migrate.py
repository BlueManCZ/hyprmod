...
class TestAutoMigrateOnWrite:
    def test_renames_deprecated_blur_keys(self, gui_conf_tmp):
        ...
        config.write_all({"decoration:blur_size": "8", "decoration:blur_passes": "2"})
    ...
    def test_renames_no_cursor_warps(self, gui_conf_tmp):
        ...
        config.write_all({"cursor:no_cursor_warps": "true"})
        content = gui_conf_tmp.read_text()
    ...
    def test_leaves_current_keys_untouched(self, gui_conf_tmp):
        ...
        config.write_all({"general:gaps_in": "5", "decoration:blur:size": "8"})
        content = gui_conf_tmp.read_text()
    ...
    def test_migrates_exec_once_in_bind_section(self, gui_conf_tmp):
        ...
        """
        config.write_all({}, env_lines=["exec_once = kitty"])
        content = gui_conf_tmp.read_text()
    ...
    def test_logs_applied_migration(self, gui_conf_tmp, caplog):
        ...
        with caplog.at_level(logging.INFO, logger="hyprmod.core.config"):
            config.write_all({"decoration:blur_size": "8"})
    ...
    def test_logs_deprecation_without_auto_migration(self, gui_conf_tmp, caplog):
        ...
        """
        with caplog.at_level(logging.INFO, logger="hyprmod.core.config"):
            config.write_all({"general:max_fps": "144"})
    ...
    def test_preserves_file_structure_headers(self, gui_conf_tmp):
        ...
        config.write_all(
            ...
            bind_lines=["bind = SUPER, T, exec, kitty"],
        )
        content = gui_conf_tmp.read_text()
...
class TestAutoMigrateContentHelper:
    """Direct tests for _auto_migrate_content — the pure-string helper."""

    def test_returns_original_when_no_migration_needed(self):
        content = "# Generated by HyprMod\n\ngeneral:gaps_in = 5\n"
        assert config._auto_migrate_content(content) is content

    def test_rewrites_when_migration_applies(self):
        content = "decoration:blur_size = 8\n"
        out = config._auto_migrate_content(content)
        assert "decoration:blur:size = 8" in out
        assert "blur_size" not in out

    def test_survives_malformed_input(self, caplog):
        """Lenient parsing + exception guards mean malformed input falls through."""
        # Nonsense that lenient parsing will accept as ErrorLine nodes.
        content = "= = = not a real line\n"
        out = config._auto_migrate_content(content)
        # The key guarantee: we don't raise and we don't lose content.
        assert "not a real line" in out
...
def test_write_all_target_is_overridable(tmp_path, monkeypatch):
    """Sanity: the fixture mechanism itself works — write_all respects set_gui_conf."""
